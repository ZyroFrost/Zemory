...
function cmdMigrate(): void {
  ...
  console.log("  4. gộp todo → 02_TODO; zemory docs rm <path> cho bản trùng/thừa (HỎI user nếu còn nội dung)");
...
function cmdArchive(): void {
  ...
  if (r.moved === 0) {
    console.log(
      `zemory archive: nothing to do (03_CHANGES.md = ${r.activeLines} lines, under threshold).`,
    );
  } else {
    console.log(`zemory archive: marked ${r.moved} old entr(ies) archived in global_memory.db.`);
    console.log(`  active 03_CHANGES.md now ${r.activeLines} lines (history remains searchable).`);
  }
}
...
function cmdStructure(): void {
  console.log(
    [
      "zemory — standard repo layout (target to conform to). Full spec: 01_RULES §Cấu trúc repo.",
      "",
      "  Always 2 surfaces: backend/ + frontend/  (root stays lean).",
      "  Source = tracked INPUT; build output + data + secret + .env = GITIGNORED.",
      "",
      "  Edit what            → where",
      "  ------------------------------------------------------------------------",
      "  UI / assets          → frontend/          any stack; assets → frontend/assets/",
      "  logic / API / auth   → backend/           own code + entry (backend/<pkg>/ | src/); security = code, not a folder",
      "  type shared BE↔FE    → backend/src/types/ FE imports from backend (no separate shared/ folder)",
      "  config app tự quản   → backend/infra/     server-side = a branch of backend",
      "  external code        → external/          cloned repos for reference; call, don't inline",
      "  backup / pre-deploy  → attic/             old/removed source + snapshot BEFORE 'up server' (tracked, rollback)",
      "  data/log/secret      → data/              .db log/cache/state/.key/bundle — gitignored",
      "  docs / rules / plan  → docs/              via `zemory` commands (don't hand-edit mirrors)",
      "  test (IF automated)  → backend/test/ (+ frontend/test/)  [OPTIONAL] — TEST usually = run the app itself",
      "  tool-forced          → ROOT              Dockerfile · compose · *.spec · .github/ (CI) · .vscode/ · pyproject/package.json",
      "  env config           → ROOT              .env.example (tracked) + .env (secret → gitignored; dotenv reads ./.env)",
      "  build output         → ROOT + .gitignore [OPTIONAL] dist/ build/ (only if built) · node_modules/ · .venv/",
      "",
      "  Required: backend/ (code) + frontend/ + docs/ + AGENTS.md.",
      "  Optional: backend/{test,scripts,infra,migrations} · frontend/test · external/ · attic/ · data/ · dist,build,.venv.",
      "  TEST = run the app itself (build a bit, look at it); a test/ folder ONLY when there are automated tests for a logic core (e.g. zemory).",
      "  Names flex per stack: Python backend/<pkg>/ · Node backend/src/ (or src/).",
      "",
      "  Refactor an app to this → AGENTS.md §8   ·   drift check → `zemory validate`",
      "",
      "docs harness (DB-source; .md are render mirrors):",
      "  docs/agent/01_RULES.md      — luật bất dịch (markdown source)",
      "  docs/agent/02_TODO.md       — backlog (mirror)",
      "  docs/agent/03_CHANGES.md    — changelog (mirror, source = changelog table)",
...
async function cmdDocs(args: string[]): Promise<void> {
  ...
  if (sub === "render") {
    ...
    const ch = renderChangelog(root, join(root, "docs", "agent", "03_CHANGES.md"));
...
async function cmdChangelog(args: string[]): Promise<void> {
  const sub = args[0];
  const root = findProjectRoot() ?? process.cwd();
  const mdPath = join(root, "docs", "agent", "03_CHANGES.md");

  if (sub === "import") {
    const n = importChangelog(mdPath, root);
    console.log(`zemory changelog import — ${n} entr(ies) from 03_CHANGES.md into global_memory.db`);
    return;
  }
  if (sub === "ls") {
  ...
  console.log(
    [
      ...
      "",
      "  import           seed global_memory.db from docs/agent/03_CHANGES.md",
...
function cmdHelp(): void {
  console.log(
    [
      ...
      "  archive   move old 03_CHANGES blocks to archive/ when over threshold",
