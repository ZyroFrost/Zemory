...
function cmdStructure(): void {
  console.log(
    [
      ...
      "",
      "  Always 2 surfaces: backend/ + frontend/  (root stays 2–3 folders).",
      "  Source folders = tracked INPUT; build output = root, GITIGNORED.",
      "",
      "  Edit what         → where",
      "  ------------------------------------------------------------------------",
      "  UI / assets       → frontend/          any stack (HTML/React/Vue…); assets → frontend/assets/",
      "  logic/API/auth·test·script → backend/  own code + entry + backend/test/ + backend/scripts/",
      ...
      "  data/log/secret    → data/             .db log/cache/state/.key/bundle — root + .gitignore",
      "  backup old/removed → attic/            old or removed source kept for reference (tracked)",
      ...
      "",
      "  Required: backend/ (code + test/ + scripts/) + frontend/ + docs/ + AGENTS.md.",
      "  Optional: external/ · data/ · attic/ · backend/infra/ · dist/,build/,.venv/ (build output).",
