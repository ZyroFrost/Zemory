// Ô "địa chỉ máy cần nối" nhận ĐÚNG chuỗi mà bề mặt in ra. Bề mặt in `10.101.1.2:21038` thành MỘT dòng,
...
test("dán nguyên dòng bề mặt in ra ⇒ tách đúng host và cổng", () => {
  assert.deepEqual(parsePeerAddress("10.101.1.2:21038"), { host: "10.101.1.2", port: 21038 });
  assert.deepEqual(parsePeerAddress("  192.168.1.90:22000  "), { host: "192.168.1.90", port: 22000 });
...
test("thiếu cổng ⇒ mặc định của sản phẩm; cổng RỜI vẫn nhận (đường cũ)", () => {
  assert.deepEqual(parsePeerAddress("10.101.1.2"), { host: "10.101.1.2", port: 21038 });
  assert.deepEqual(parsePeerAddress("10.101.1.2", 22000), { host: "10.101.1.2", port: 22000 });
  // cổng trong CHUỖI thắng cổng rời — người dán biết rõ hơn giá trị còn sót trong ô
  assert.deepEqual(parsePeerAddress("10.101.1.2:21038", 22000), { host: "10.101.1.2", port: 21038 });
});
...
test("CA ÂM: dán sai thì BÁO, không lặng lẽ nối sang cổng khác", () => {
  ...
  assert.equal(parsePeerAddress("   "), null);
  assert.equal(parsePeerAddress("10.101.1.2:cong"), null, "cổng không phải số");
  assert.equal(parsePeerAddress("10.101.1.2:70000"), null, "cổng ngoài 1–65535");
  assert.equal(parsePeerAddress("10.101.1.2:0"), null);
