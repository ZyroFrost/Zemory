...
test("🔴 IPv4 ánh xạ IPv6 — dạng dò LAN thật sự trả về, và là ca luật IPv6 trả lời SAI", () => {
  ...
  // KHÔNG lần nào gọi thẳng được. `rinfo.address` của socket hai tầng trả `::ffff:203.0.113.29`,
  // nên ứng viên dựng ra là `::ffff:203.0.113.29:21038` — và luật ngay trên (*nhiều dấu `:` không
  ...
  assert.deepEqual(parsePeerAddress("::ffff:203.0.113.29:21038"), { host: "203.0.113.29", port: 21038 });
  assert.deepEqual(parsePeerAddress("::ffff:10.0.3.26"), { host: "10.0.3.26", port: 21038 }, "không cổng ⇒ rơi về mặc định");
  assert.deepEqual(parsePeerAddress("::FFFF:203.0.113.29:21038"), { host: "203.0.113.29", port: 21038 }, "hoa/thường đều là cùng một dạng");
  ...
  // loại ra, chứ không được nắn thành `203.0.113.300` — một host trông hợp lệ mà không tồn tại.
  assert.equal(parsePeerAddress("::ffff:203.0.113.300:21038"), null, "octet ngoài dải ⇒ từ chối, đừng đoán");
});
...
test("🔴 dò LAN nắn địa chỉ NGUỒN ngay tại gốc — nơi duy nhất dạng đó ra đời", async () => {
  ...
  assert.equal(normalizeSourceHost("::ffff:203.0.113.29"), "203.0.113.29");
  ...
  assert.equal(normalizeSourceHost("203.0.113.29"), "203.0.113.29");
