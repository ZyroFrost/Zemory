...
test("a HUGE tool_result is stored whole - the 4,000 cap is gone", () => {
...
test("a huge tool_use input is not truncated either", () => {
  const p = claudeAdapter.parseLine(
    line({
      type: "assistant",
      uuid: "u2",
...
test("a long text block is kept intact (the ChatGPT-style whole-message cut is removed)", () => {
  const p = claudeAdapter.parseLine(
...
test("a FILE the user dropped into the chat becomes a message with its full content", () => {
...
test("Claude Code INTERNAL attachments are dropped (no junk ingested)", () => {
...
test("an EMPTY attachment file is dropped", () => {
  const p = claudeAdapter.parseLine(
...
test("the 4 filter layers that HAVE A REASON are still there: blank line - broken JSON - thinking - empty message", () => {
...
test("custom-title still beats ai-title (the attachment branch did not break it)", () => {
...
test("an image block does NOT enter content - it leaves one label line", () => {
...
test("images land in `attachments` with sha256 + kind=blob", () => {
...
test("the same image yields the same sha256 (the basis of dedup)", () => {
...
test("a broken or unknown image block is skipped WITHOUT losing the whole message", () => {
