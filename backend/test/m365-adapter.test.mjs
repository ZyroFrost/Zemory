...
test("① câu trả lời của bot LẤY TỪ adaptiveCards khi `text` rỗng", () => {
...
test("② tin Progress được GIỮ nhưng đóng dấu toolName để hạ hạng", () => {
  const [s] = m365CopilotAdapter.parseFileMulti(file);
...
test("③ author 'bot' → assistant · 'user' → user · id phiên mang tiền tố của nền", () => {
  const [s] = m365CopilotAdapter.parseFileMulti(file);
...
test("④ tin CHỈ có ảnh giữ lại NHÃN — không im lặng đánh rơi", () => {
  const [s] = m365CopilotAdapter.parseFileMulti(file);
  assert.ok(
...
test("⑤ tiền tố phiên của adapter KHỚP `sessionPrefix` khai trong PLATFORMS", async () => {
...
test("⑥ file không đọc được / không phải hội thoại ⇒ null, KHÔNG ném", () => {
...
test("⑦ sidecar `_pulled.json` KHÔNG bị nhặt làm transcript", () => {
