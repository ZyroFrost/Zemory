...
import { relocateBrain, storageInfo, looksLikeCloudSync } from "../../dist/brain/relocate.js";
import { tempDir } from "./helpers.mjs";

/** Build a throwaway brain at <dir>/global_memory.db with N messages; return paths. */
function seed(t, n = 5) {
  const home = tempDir(t, "zloc-home-");
  const src = tempDir(t, "zloc-src-");
  const db = join(src, "global_memory.db");
  const conn = openBrain(db);
...
test("relocateBrain moves + verifies the DB, writes the pointer, keeps a .bak", (t) => {
  const paths = seed(t, 7);
  const dst = join(tempDir(t, "zloc-dst-"), "data");

  const r = relocateBrain(dst, { paths });
  ...
  const moved = openBrain(join(dst, "global_memory.db"));
  assert.equal(moved.prepare("SELECT COUNT(*) c FROM messages").get().c, 7);
  moved.close();
});

test("relocateBrain refuses a cloud-synced target unless forced", (t) => {
  ...
  assert.throws(() => relocateBrain("G:\\My Drive\\zemory", { paths }), /cloud-synced/i);
  // old DB untouched after a refusal
  assert.ok(existsSync(paths.db));
});

test("relocateBrain with no DB yet only sets the pointer (pointerOnly)", (t) => {
  const home = tempDir(t, "zloc-home-");
  const src = tempDir(t, "zloc-src-");
...
test("relocateBrain is disabled while GLOBAL_MEMORY_DB pins the location", (t) => {
  const paths = { ...seed(t, 2), pinned: true };
  assert.throws(() => relocateBrain(join(tempDir(t, "zloc-dst-"), "data"), { paths }), /pins the DB location/i);
});
