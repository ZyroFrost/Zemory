...
function listFeatures(): FeatureStatus[] {
  ...
  return [
    ...
    { key: "profile-reclaim", group: "workflow", label: tr("Tự thu hồi profile trình duyệt cũ", "Reclaim stale browser profiles"), state: "idle", detail: "—",
      help: tr("Khi trình duyệt mặc định của máy đổi, zemory KHÔNG xoá profile web cũ mà dời sang bên (`<khe>.<hãng>-bak-<epoch>`) — khe đó phải đăng nhập lại một lượt. Không dòng code nào lấy lại bản dời sang bên đó (quay về hãng cũ vẫn tạo profile MỚI), nên nó là rác kể từ lúc sinh ra, và nó nằm dưới `data/` đã gitignore nên không cổng nào thấy nó lớn lên (đo 2026-08-31: 12 thư mục · 3,4 GB, cũ nhất 27 ngày, so với 4 khe đang sống chỉ 579 MB). Daemon thu hồi bản quá 7 ngày ở lượt quét rác 6 giờ; CHỈ đụng thứ app tự sinh (`-bak-`), không bao giờ đụng khe đang sống hay bản người/agent lưu tay.", "When the machine's default browser changes, zemory does NOT delete the old web profile — it moves it aside (`<slot>.<brand>-bak-<epoch>`) and that slot needs one fresh login. Nothing in the code ever restores an aside copy (switching back still builds a NEW profile), so it is garbage from birth, and it lives under gitignored `data/` where no gate sees it grow (measured 2026-08-31: 12 dirs · 3.4 GB, oldest 27 days, against 579 MB for the 4 live slots). The daemon reclaims copies older than 7 days on the 6-hour sweep; it only ever touches app-created `-bak-` dirs, never a live slot or a hand-made copy.") },
