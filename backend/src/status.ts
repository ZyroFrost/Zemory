...
function listFeatures(): FeatureStatus[] {
  ...
  return [
    ...
    { key: "paths", group: "workflow", label: tr("Đường dẫn chết trong docs", "Dead paths in docs"), state: "idle", detail: "—",
      help: tr("Theo dõi đường dẫn viết trong docs/config của project: lần đầu ghi baseline, sau đó chỉ báo đường MỚI chết (còn sống lúc baseline, nay không tồn tại) — tức đúng lúc một folder bị dời/đổi tên. Chỉ phán đường nằm dưới root đã khai (`pathCheck.roots`, mặc định = gốc repo); ngoài root thì liệt kê, không kết luận. `zemory paths check`.", "Watches the file paths written in the project's docs/config: the first run records a baseline, then reports only paths that died AFTER it (alive at baseline, missing now) — i.e. the moment a folder was moved or renamed. Only paths under declared roots are judged (`pathCheck.roots`, default = repo root); anything else is listed, not judged. `zemory paths check`.") },
