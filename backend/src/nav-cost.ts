...
// folder tree + code graph + global brain let an agent go STRAIGHT to the right
...
// repo, both sides measured from real bytes on disk / real rows in the brain:
//   • sweep  — read everything you'd have to read to answer the question blind
//   • routed — read only what the index / graph / brain hands back
...
// an unreadable brain degrades that one lane to `available: false`, never throws.
...
export interface NavLane {
  ...
  ratio: number;
  /** False when this lane can't be measured here (missing docs / empty brain). */
  available: boolean;
...
export interface NavCost {
  ...
  impact: NavLane;
  /** "what happened in earlier sessions?" — brain recall vs re-reading them. */
  recall: NavLane;
...
function priorContext(root: string, dbPath?: string): { chars: number; sessions: number } {
  try {
    ...
    return { chars: 0, sessions: 0 }; // brain optional — fail open (HP điều 9)
  }
}
