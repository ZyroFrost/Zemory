  ...
  try {
    ...
    sock.on("message", (buf, rinfo) => {
      try {
        ...
        const m = JSON.parse(text.slice(MAGIC.length)) as { id?: string; port?: number };
        ...
        const p: PeerSighting = { deviceId: m.id, host: rinfo.address, port: m.port, seenAt: new Date().toISOString() };
    ...
    const beat = (): void => {
      ...
      const payload = Buffer.from(`${MAGIC}${JSON.stringify({ id: o.deviceId, port: o.channelPort })}`, "utf8");
      try {
