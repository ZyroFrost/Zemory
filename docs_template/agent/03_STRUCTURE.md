...
> ① **APP** (có code chạy: UI/server/CLI) → **§1–6**. ② **NON-APP** (sản phẩm/tài sản: BI/report, data, docs-only, design — vd `powerbi_duana`) → **§7**. Cả hai dùng CHUNG harness `docs/` + `AGENTS.md`.
...
## 6. Phạm vi áp dụng
- **ÁP §1–6 (chuẩn APP):** hầu hết app estate (UI + server-side) — desktop WebView2 (DuAnA), web app, tool có cockpit (zemory), AI project, monorepo. Áp gần như không đổi cấu trúc; chọn layer-first hay domain-first theo số domain (§2).
- **ÁP §7 (chuẩn NON-APP):** project là SẢN PHẨM/TÀI SẢN, không phải code chạy — BI/report (Power BI/Tableau, vd `powerbi_duana`), data/analytics (dbt/warehouse), docs-only, design/brand, research/notebook có cấu trúc.
...
```
<project>/                         # vd powerbi_duana — 1 sản phẩm = 1 cây
...
**Ví dụ áp `powerbi_duana`** — chỉ hiện slot CÓ THẬT (không tạo folder rỗng):
```
powerbi_duana/
├── AGENTS.md
...
├── assets/duana-theme.json                   # theme màu
