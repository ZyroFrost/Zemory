...
EOL của file         ⛔ KHOÁ BẰNG MÁY TRƯỚC, kỷ luật sau — repo phải có `.gitattributes` với `* text=auto eol=lf` (đặt TRƯỚC các dòng LFS: gitattributes lấy dòng SAU đè dòng TRƯỚC, nên `-text` của `.pbix`/`.xlsx` mới thắng được `text=auto`). Có khoá rồi thì git tự chuẩn hoá và không phiên nào lệch được nữa. **Đo một estate thật 2026-09-21: 8 repo đều có `.gitattributes` nhưng chỉ 3 có dòng khoá — 5 repo còn lại vẫn trôi, vì trước đó chỉ có LUẬT CHỮ chứ không có chốt máy.** CHƯA khoá thì áp luật dưới: ⛔ ĐỌC SAO GHI VẬY: sửa file CÓ SẴN thì giữ nguyên kiểu xuống dòng của nó — đọc dạng BYTE, thấy CRLF thì ghi lại CRLF. Đọc thành text rồi ghép lại bằng LF là ÂM THẦM đổi cả file sang LF ⇒ git kêu MỌI DÒNG đều đổi, thay đổi thật chìm trong nhiễu và mất blame theo dòng. Đo 2026-09-04: 2 file bị vậy, và cùng lượt ghi-lại-cả-file đó còn LÀM RƠI phần phiên khác vừa chèn (lost update, không ai báo lỗi) ⇒ sửa theo TỪNG ĐOẠN, đừng đọc-cả-file-rồi-ghi-lại
