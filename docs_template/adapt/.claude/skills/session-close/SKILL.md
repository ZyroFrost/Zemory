...
3. **SOÁT `05_TODO` = ĐO LẠI, KHÔNG đọc rồi chép** (`02_RULES §Hành xử` — luật áp **MỌI LÚC**, chốt phiên chỉ là một trường hợp): mỗi mục là một **KHẲNG ĐỊNH VỀ TRẠNG THÁI**, mà khẳng định phải truy được về nguồn kiểm được. Kiểm được bằng code ⇒ **grep/chạy/đếm**; là quyết định ⇒ **`memory search --all`** (quyết định hay nằm ở phiên khác, thậm chí **repo khác**); tra không ra ⇒ ghi **"chưa xác minh được"**, KHÔNG mặc định "chưa làm". **Mục quá 7 ngày không ai đụng = NGHI NGỜ.** Hỏi lại user một việc đã chốt là **LỖI**, không phải cẩn thận.
