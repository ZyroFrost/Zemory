...
| **daemon nền / server tiến-trình-dài** phục vụ UI cục bộ (single-instance, cổng cố định) | surface mỏng ở `backend/src/` mở endpoint `api/` + serve `frontend/` tĩnh; nghiệp vụ vẫn ở domain |
| **engine graph** (import-graph · symbol · docs-graph · touches) | LỒNG trong domain (`backend/src/<domain>/`), KHÔNG slot thứ 5 (theo điều hiến pháp về slot graph, nếu project có khai); consumer ngoài đọc `graph export` versioned |
...
| **nơi lưu DB local + dời off ổ hệ thống** | DB ở `data/*.db` (hoặc thư mục app-data `~/.<tool>/`); con trỏ CỐ ĐỊNH `~/.<tool>/location.json` quyết định vị trí → dời bằng lệnh relocate của tool / UI "Nơi lưu". **KHÔNG** để DB sống trong folder Drive-sync (WAL corrupt) |
...
Nơi lưu DB (di dời)  DB sống lớn dần → cho dời KHỎI ổ hệ thống. Vị trí = con trỏ CỐ ĐỊNH ~/.<tool>/location.json (env override > pointer > ~/.<tool> default); mọi phụ trợ (config/browser/imports/backups) bám theo. Dời an toàn = checkpoint WAL → copy → verify (integrity + đếm) → đổi pointer → GIỮ bản cũ .bak. Con trỏ PHẢI ở chỗ cố định, KHÔNG cạnh DB (phụ thuộc vòng). KHÔNG dời vào folder Drive-sync (WAL corrupt) — Drive chỉ nhận bundle .enc
