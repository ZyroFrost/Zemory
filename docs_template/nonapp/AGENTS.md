...
1. **Chưa có `docs/.harness.json`?** →
   ...
   - Rồi chạy: **APP** → `zemory init` · **NON-APP** → `zemory init --non-app` → `zemory doctor` xanh. (Thiếu lệnh `zemory` → `npm i -g zemory`. Mạng chặn npm thì cài từ mã nguồn: `git clone https://github.com/ZyroFrost/Zemory` → `npm install` → `npm run build` → `npm link`. **Đừng dùng `npm i -g github:…`** — cài global không kéo devDependencies nên thiếu `tsc` để dựng.) Đã có `.harness.json` → bỏ qua bước này (profile đã chốt trong đó).
