...
1. **Chưa có `docs/.harness.json`?** →
   ...
   - Rồi chạy: **APP** → `zemory init` · **NON-APP** → `zemory init --non-app` → `zemory doctor` xanh. (Thiếu lệnh `zemory` → cài TỪ MÃ NGUỒN: `git clone https://github.com/ZyroFrost/Zemory` → `npm install` → `npm run build` → `npm link`. **`npm i -g zemory` KHÔNG chạy** — gói chưa publish lên npm. **`npm i -g github:…` cũng không** — cài global không kéo devDependencies nên thiếu `tsc`.) Đã có `.harness.json` → bỏ qua bước này (profile đã chốt trong đó).
