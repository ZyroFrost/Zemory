...
1. **Chưa có `docs/.harness.json`?** →
   ...
   - Rồi chạy: **APP** → `zemory init` · **NON-APP** → `zemory init --non-app` → `zemory doctor` xanh. (Thiếu lệnh `zemory` → cài TỪ MÃ NGUỒN: `git clone https://github.com/ZyroFrost/Zemory` → `npm install` → `npm run build` → `npm link`. **KHÔNG dùng `npm i -g zemory`** — chưa publish lên npm, và `npm i -g github:…` cũng hỏng vì cài global không kéo devDependencies nên thiếu `tsc`; đo thật 2026-08-03.) Đã có `.harness.json` → bỏ qua bước này (profile đã chốt trong đó).
...
> Chúng cộng lại dài hơn cả bộ luôn-nạp, mà mỗi phiên chỉ dùng tới một hai cái — nạp trước cho "chắc"
> là đốt ngữ cảnh trước khi bắt đầu làm việc. **Mở một skill = đọc NGUYÊN file đó**, không đọc lướt;
...
| đẩy deliverable lên đích (BI · Drive · SharePoint) | `.claude/skills/upload/SKILL.md` |
