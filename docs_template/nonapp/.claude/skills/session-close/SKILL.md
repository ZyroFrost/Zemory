...
3. **SOÁT `05_TODO` = ĐO LẠI, KHÔNG đọc rồi chép** (`02_RULES §Hành xử` — luật áp **MỌI LÚC**, chốt phiên chỉ là một trường hợp): mỗi mục là một **KHẲNG ĐỊNH VỀ TRẠNG THÁI**, mà khẳng định phải truy được về nguồn kiểm được. Kiểm được bằng file/số liệu ⇒ **mở ra đếm**; là quyết định ⇒ **`memory search --all`** (quyết định hay nằm ở phiên khác, thậm chí **repo khác**); tra không ra ⇒ ghi **"chưa xác minh được"**, KHÔNG mặc định "chưa làm". **Mục quá 7 ngày không ai đụng = NGHI NGỜ.** Hỏi lại user một việc đã chốt là **LỖI**, không phải cẩn thận.
4. **Áp CẢ cho audit / báo cáo lỗi:** mỗi finding phải đối chiếu file THẬT + GM trước khi gọi là "lỗi thật". Bẫy điển hình: tên file cũ trong entry changelog CŨ là **BẢN GHI LỊCH SỬ**, KHÔNG phải link gãy cần sửa; số liệu trong report cũ có thể là snapshot đúng-tại-thời-điểm. KHÔNG tin kết quả subagent chưa tự kiểm lại.
