...
1. `tasks/<tên_case>/` — **tên THƯỜNG, KHÔNG đánh số**, kể cả case định kỳ. Quy ước đặt tên là của repo (vd một estate dùng `<MÃ PHÒNG>_<NHỊP>_<TênViệc>` như `IT_Weekly_Inventory`); chuẩn không ép tên, chỉ ép **không có số thứ tự**. ⚠ Bãi bỏ vế cũ *“số `NN_` cho case định kỳ”*: số trên tên case là số chết (case không chạy theo thứ tự, thêm/bỏ là đánh số lại cả dãy) và nó lẫn với số STAGE trong `pipeline/` của case vốn có nghĩa thật.
