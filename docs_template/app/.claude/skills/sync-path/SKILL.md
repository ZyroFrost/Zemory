...
fix*, không phải một đường cố định. Đã trả giá thật: bỏ **hàng chục giờ máy** dựng một lớp chỉ
mục, rồi mới lộ ra gói đồng bộ mặc định **không chở lớp đó**; và lệnh xuất gói bàn giao **không
tự tìm chìa** trong khi chìa nằm ngay cạnh kho.
...
|---|---|---|
| **git** | mã nguồn · docs · template · hook · cấu hình mặc định | **cấm mọi data/bí mật** (xem `01_CONSTITUTION`) |
| **gói mã hoá** | kho dữ liệu thật | không bao giờ qua git; không để kho SỐNG trong thư mục đồng bộ đám mây |
| **người mang tay** | chìa · mật khẩu | không git, không đám mây, không in ra màn hình |
| **dựng lại tại máy đích** | lớp dẫn xuất rẻ · output build · cache | phải ghi rõ **lệnh dựng** và **giá** (bao lâu) |
| **tải lúc chạy** | model weight · asset nặng | không commit — nhớ nó có thể cần cả lúc DÙNG, không chỉ lúc dựng |
...
# 3. mở bản tạm ra và ĐẾM: phép kiểm toàn vẹn · số bản ghi · từng lớp chỉ mục · cấu hình chỉ mục
```

Đếm phải phủ **cả lớp dẫn xuất**, vì đó chính là lớp hay bị rơi. Một con số tổng ("2 GB") không
nói được gì — nó vẫn đúng khi phần lớn dung lượng là thứ bên nhận sẽ vứt.
...
*Đã trả giá thật:* gói bàn giao nằm im trong thư mục đồng bộ, **hai gói 317 MB kẹt suốt 3 ngày**
mà không ai biết — trong khi lệnh đồng bộ vẫn báo "đã xuất" thành công mỗi lần. Đúng kiểu hỏng mà
`02_RULES §Bề mặt CHẾT THEO nền` gọi là **vỏ rỗng: không báo lỗi, nó nói dối**.

## 4. Bẫy đã trả giá — đọc trước khi tự tin

- **Hai đường tên gần giống nhau, hành vi khác hẳn.** *Gộp thêm* (merge) có thể chỉ lấy bảng NGUỒN;
...
- **Mỗi cửa vào một kiểu dò chìa.** Một lệnh tự dò, lệnh kia bắt truyền tay ⇒ người làm theo tài
  liệu vẫn thất bại. **Mọi cửa của cùng một đường phải dò giống nhau.**
...
- **Mẫu gitignore không neo.** Mẫu trần khớp MỌI độ sâu, từng nuốt luôn thư mục dữ liệu của thư
  viện vendored ⇒ ai clone về cũng nhận bản cụt ruột. Neo ở gốc (`/data/`, không phải `data/`).
- **Model weight / binary nặng**: cấm commit (từng làm nghẽn push vì vượt giới hạn 100 MB của
  GitHub), nhưng phải NÓI RÕ máy đích tự tải — và rằng thiếu nó thì lớp phụ thuộc nó **im lặng**
  rơi về mức thấp hơn.

## 5. Máy canh, đừng dựa trí nhớ

Skill là chữ, mà chữ thì quên được — thứ CHẶN drift là **code**. Repo phải có cổng cho ít nhất hai
việc, và **chạy chúng thay vì tự nhớ**:

- cổng chặn **data/bí mật lọt git** — kèm ca ÂM: một ngoại lệ mở đường cho gói dữ liệu là đủ hỏng;
- cổng canh **mọi cửa của đường đồng bộ dò chìa giống nhau**.
