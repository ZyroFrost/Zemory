...
> ⚠ **Cài bằng cách nào:** `npm i -g zemory` **KHÔNG chạy** — công cụ này *chưa publish lên npm*.
> Phải cài từ mã nguồn: `git clone` → `npm install` → `npm run build` → `npm link`.
...
npm link). Đừng dùng `npm i -g zemory`, gói đó không có trên npm.
```
