...
- **Test mới phải chứng minh mình ĐỎ ĐƯỢC.** Viết xong một test, hãy **phá code mà nó canh** rồi chạy lại: không đỏ ⇒ test đó chưa soi gì, phải sửa test chứ không phải mừng vì xanh. Hai lỗ đã bắt được đúng bằng cách này: một test chưa bao giờ chạy tới nhánh nguy hiểm, một test bị **bản sao logic ở nơi khác gánh thay**. Xanh KHÔNG phải bằng chứng (xem `04_SKILLS §audit toàn diện` luật 1).
