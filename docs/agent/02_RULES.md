...
- **code · comment: TIẾNG ANH TOÀN BỘ** — comment · docstring · tên biến/hàm · log/thông điệp kỹ thuật · commit message. Một file một ngôn ngữ (đo repo thật 2026-08-29: một file trộn ba kiểu VN-không-dấu · VN-có-dấu · English). **Ba thứ KHÔNG phải comment — KHÔNG dịch:** ① **chuỗi RENDER ra tài liệu / bề mặt người đọc** (desc·note sinh vào `docs/`, nhãn UI qua i18n) theo luật của docs/UI ở trên, không theo luật code · ② **tên do người khác đặt** (tên cột Excel · trường API · tên file nguồn — vd `"Kich thuoc khu dat"`) giữ NGUYÊN từng ký tự, dù trông như "Việt mất dấu": đổi là loader không tìm thấy · ③ **thuật ngữ chuyên ngành** giữ nguyên. Comment Việt cũ: sửa khi **đang đụng file đó**, KHÔNG mở chiến dịch dịch hàng loạt (đo: 20.203 token comment dịch hết tiết kiệm ~4.000 token/lần đọc toàn repo — không đáng một đợt riêng).
...
- **MỖI LẦN PUSH = MỘT LẦN LÊN VERSION — và SỐ do USER chốt (user chốt 2026-08-05).** Push là mốc phát hành (khớp luật Version release-based ở §Luật khi VIẾT): trước khi đẩy, agent ① kiểm file sạch (gate/lint những gì chạy được, `git status` không lọt data/secret) + rà `05_TODO` còn gì phải đóng, ② **HỎI user số version** (đề xuất theo semver: minor=tính năng, patch=fix — nhưng quyền chốt là của user), ③ bump `package.json` + commit + push trong cùng một mốc. KHÔNG push với số cũ, KHÔNG tự quyết số.
  - **TRỪ đợt CHỈ ĐỤNG DOCS — đẩy thẳng, KHÔNG bump** *(user chốt 2026-09-04: "khỏi đi, mấy cái fix ko liên quan gì app")*. Vế "mỗi lần push = một lần lên version" viết cho đợt có **BẢN BUILD** đi kèm; docs không sinh artifact nào, `dist/` byte-identical, và semver ở §Luật khi VIẾT chỉ định nghĩa `minor`=tính năng · `patch`=fix deploy — docs không rơi vào ô nào. **Thông lệ đã vậy sẵn** (đo 2026-09-04: sau `715f4eb` có **4** commit docs-only push mà không bump). Và bump cho docs có giá THẬT: `publishChannelVersion` đóng dấu `appVersion()` lên `<Drive>/version.json` ⇒ báo cho máy kia là **có bản mới** trong khi bản build y hệt, làm nhiễu đúng tín hiệu `selfupdate` đang cần. Đụng tới `backend/` · `frontend/` · `package.json` ⇒ quay về luật gốc, phải hỏi số.
