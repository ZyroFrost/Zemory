...
6. **zemory KHÔNG BAO GIỜ tự gọi LLM / không proxy model API.** Không `ANTHROPIC_BASE_URL`, không rewrite history/cache, không sinh văn bản. "Trí tuệ" là agent đang lái terminal; zemory chỉ là bộ nhớ + kỷ luật. Embed/rerank model nhỏ chạy local chỉ *đo nghĩa* (chấm điểm/xếp hạng), không *sinh văn bản* — vẫn đúng luật. **PHẠM VI (khoanh vùng chống hiểu lầm, 2026-07-25): điều này ràng buộc CHÍNH hệ zemory (memory · search · harness · graph) — KHÔNG áp cho APP mà harness dựng ra.** Chuẩn `03_STRUCTURE` cố tình có slot `ai/`·`agents/`·`tools/`·`evals/` để user/agent XÂY app LLM/AI tự do; "no-LLM" là ĐẶC TÍNH của engine bộ nhớ zemory, KHÔNG phải triết lý chống-LLM. (Nhiều agent đọc README hiểu sai thành "zemory chống app-LLM" — đã sửa README + khoanh vùng ở đây.) *(gốc: build plan §2.5 §2.7, plan 05 §2, plan 06 §2.4 §4B)*
