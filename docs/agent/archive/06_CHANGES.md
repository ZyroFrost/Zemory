...
> 🔄 **Supersede:** thay [2026-09-02b] — "mượn cookie: CÓ COOKIE ≠ CÓ PHIÊN", **vế §Ranh giới**
> (*"KHÔNG chép cookie `accounts.google.com`… header file cấm đúng ca này"*) — user chốt đổi sau khi
> nghe rõ đánh đổi. Cùng lượt bãi bỏ luật header `borrowcookies.ts` *"borrows ONE site's cookies,
> not the jar"*. Dấu này thêm 2026-09-03 khi chốt phiên: entry gốc thiếu nó, mà vế bị thay là một
> phát biểu về BẢO MẬT — ai tra trúng `[2026-09-02b]` mà không thấy nhãn sẽ đọc nó như chính sách
> đang hiệu lực.
...
## [2026-08-24f] — cổng "bundle ĐÃ RỜI KHỎI MÁY chưa" (`uplinkguard`) — trả nợ plan/18 mặt ⑨

**Sự cố gốc (2026-08-11):** client Drive kẹt hàng đợi — gói 317 MB + bản bàn giao 1,63 GB nằm im
**3 ngày** trong thư mục Drive trong khi `memory sync` lượt nào cũng báo "đã xuất" thành công;
máy kia không nhận được gì và không cổng nào đỏ. User gật 24/08, làm phiên này.

**Cách đo — sổ của client, KHÔNG tin chữ log:** `uplinkguard.ts` đọc `metadata_sqlite_db` của
DriveFS (readonly; schema ĐO THẬT trước khi viết: `items.id` còn tiền tố `local-` = chưa lên mây,
thế hệ cũ cùng tên nằm lại với `trashed=1` — 5/6 hàng `global_memory.enc` là xác cũ). **Phải đối
chiếu file CÒN TRÊN ĐĨA**: sổ sống giữ 17 hàng `local-` của `.zemory-write-probe` đã xoá từ lâu —
chỉ đọc sổ là báo oan ngay ca đầu. Ngưỡng 60′ (`ZEMORY_UPLINK_STALE_MIN`), > 1 nhịp autosync và
đủ cho gói vài GB; sự cố thật là 3 ngày nên ngưỡng rộng không làm mù cổng.

**Hai bề mặt cùng ăn:** `doctor` thêm dòng `uplink:` (✗ kẹt = gate đỏ · ○ đang lên/chưa kiểm được
· ✓ kèm số file sổ xác nhận) · `memory sync` cảnh báo ngay sau khi in "đã xuất" nếu bundle các
lượt TRƯỚC vẫn chưa rời máy — đúng chỗ câu nói dối cũ đứng. Ràng buộc user giữ đủ: chỉ-đọc sổ ·
fail-open ba tầng (không sổ / không hàng / không đọc được thư mục — đều NÓI ra, không nuốt).

**Nghiệm thu:** `uplink-guard` **9/9** · **đột biến 2 hướng đều ĐỎ** (mù `local-` ⇒ 3 đỏ · bỏ lọc
trashed + lấy hàng cũ ⇒ 1 đỏ) · chạy thật trên sổ sống: `uplink: ✓ 3 file` — khớp chéo đường hai
(`ls` đĩa: đúng 3 file) · lint sạch.

## [2026-08-24e] — PUSH 2.4.0 + áp chuẩn 9 repo một lượt (`stale: []`)

**① Release 2.4.0 — theo đúng trình tự bàn giao.** Gate chạy LẠI từ đầu (daemon tắt trước, gate
một mình): **777/777 · 0 fail · 0 skipped** (25 phút — thời gian khớp công việc) · `conform` ✓
(242 file, 0 lệch) · `todo verify` ✓ · `validate` sạch. Bump 2.3.0→2.4.0, commit `4761125`
(57 file, +3.419/−1.851, gồm 8 file mới của phiên 23–24/08), cờ `.allow-push` một-lần → push
`5c92b32..4761125` lên `origin/main`. ⚠ Cờ `.allow-push` KHÔNG tự tiêu sau push (hook không chạy
trên đường PowerShell) — đã xoá tay ngay để trả lại lưới chặn. Daemon bật lại: **pid 9500 · 2.4.0
· --no-window**, 4 công tắc đều BẬT.

**② Áp chuẩn TOÀN BỘ repo cũ — một lần, đúng lệnh user 24/08.** `/harness-updates` đo **9 repo
stale** (sổ ghi 8 — danh sách sống thắng). Từng repo `sync` → `hook guard` → `node --check` bản
guard sinh ra (bài học escape) → `doctor`:

Gap-fill: Harness AI 15 file · SasinHub 13 (cả `01_CONSTITUTION`+`04_SKILLS`) · Camera_AI ·
SasinInfra · SasinHarvest 3 mỗi repo · SasinFlow · PBI_Rebuild · PBI_HR 2 · PBI_OPS 0 (đã đủ).
Guard: sinh mới 5 repo · refresh 4 · **cả 9 qua `node --check`** (bài học escape). Doctor: 9/9 xanh
(Harness AI 1 · SasinHub 3 issue `validate` — việc của agent bên đó).

Nghiệm thu bằng chính thước máy: `/harness-updates` → **`stale: []`**. Từ nay các repo tự nhận
thay đổi qua chấm than update + `zemory selfupdate`. 4 repo chưa khai `protected` trong
`.harness.json` (Harness AI · Camera_AI · SasinInfra · SasinHub) — guard vẫn gác nhóm secret;
khai đường cấm ghi là việc của agent bên đó khi có đường cần cấm.
