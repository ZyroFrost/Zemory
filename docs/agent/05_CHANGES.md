...
**C???a ch???n cho agent c???a project KH??C gh?? v??o repo n??y.** S??? c??? n???n: session kh??c tr??? v??o zemory r???i t??? ch???y l???nh `zemory` (???? t??? revert, ki???m ch???ng s???ch) ??? c??ng h??? v???i s??? c??? t??i tr??? ng?????c sang SasinFlow. Lu???t `02_RULES ??Ph???m vi project` m???i ch??? c?? **m???t v???** (?????ng ??? project m??nh ??? c???m ghi RA ngo??i); thi???u **v??? ng?????c**: ??ang ?????NG TRONG repo tham kh???o th?? c??ng c???m ghi.

**G???c r??? d??? d??nh:** l???nh `zemory` **GHI THEO CWD**. Agent t?????ng "ch???y `zemory docs sync` ????? l???y chu???n" nh??ng ?????ng ??? repo zemory ??? ghi v??o repo zemory + DB c???a n??, kh??ng ph???i v??o project m??nh. `init`/`sync`/`docs sync`/`docs render`/`plan set`/`changelog` ?????u v???y.

**Th??m:**
1. **Banner ??? ?????u `AGENTS.md`** (c???a ?????u ti??n agent ?????c) ??? "m??? repo n??y ????? L??M VI???C hay ch??? THAM KH???O?"; n???u tham kh???o ??? CH??? ?????C, kh??ng s???a file, kh??ng ch???y `zemory` v???i cwd ??? ????y (li???t k?? ????ng c??c l???nh GHI), c???nh b??o repo c?? th??? ??ang c?? phi??n agent kh??c ??? xung ?????t th???t; l???y chu???n = **?????c `docs/agent/*` r???i ch???y l???nh ??? REPO C???A B???N**.
2. **`02_RULES ??Ph???m vi project` +v??? ng?????c** ??? n??u ????ng c?? ch??? "GHI theo cwd".
3. **Hi???n ph??p ??M???c ????ch** ghi vai th??? hai c???a repo: **ngu???n chu???n g???c ????? copy** (`docs-template/` + `docs/agent/*`) ??? agent ngo??i ch??? ?????c.

**B???y ???? n??:** `adopt.ts` **t??? refresh `AGENTS.md` t??? template** khi n?? b???t ?????u b???ng `<!-- zemory` ??? vi???t lu???t ri??ng v??o AGENTS.md c???a zemory s??? b??? l???n `sync` sau xo?? s???ch. N??n banner vi???t **generic v??o template** (????ng cho M???I repo ??? repo n??o c??ng c?? th??? b??? gh?? nh???m), b???n c???a zemory y h???t ??? refresh kh??ng ph??. Ki???m ch???ng: ch???y `zemory sync` ??? b??o `kept existing: AGENTS.md`, banner c??n nguy??n.

Gate: `npm run check` 88/88 ?? `docs sync` x??c nh???n DB nu???t ????? (section #7017 hi???n ph??p, #7025 rules).

## [2026-07-16] — feat(constitution): them muc MUC DICH bat buoc + phi-muc-tieu; don 15 header cu; va false-positive salvage

**Hi???n ph??p thi???u ch??? khai M???C ????CH** (user ch??? ra). N???ng h??n t?????ng: **KH??NG file n??o trong harness n??i project sinh ra ????? l??m g??** ??? `AGENTS.md` ch??? m?? t??? *zemory* v?? b??? `zemory sync` refresh t??? template n??n **kh??ng th???** gi??? m?? t??? ri??ng c???a app; plan m?? t??? THI???T K??? ch??? kh??ng ph???i L?? DO T???N T???I. Hi???n ph??p l?? ch??? duy nh???t ????ng vai: per-app ?? t???i cao ?? user s??? h???u ?? ?????c ?????u ti??n. M?? n?? ch??? c?? ??i???u kho???n, kh??ng c?? b???i c???nh ????? c??c ??i???u kho???n ???? ph???c v???.

**Th??m `## M???c ????ch` (B???T BU???C, ?????ng TR?????C ????i???u kho???n)** ??? c??? template l???n zemory:
- Project n??y l?? g?? / ph???c v??? ai / gi???i b??i to??n g?? (2???4 c??u, ????? ????? agent l??? n???m b???i c???nh).
- **PHI-M???C-TI??U** ??? th??? c??? t??nh KH??NG l??m; ch???ng scope creep, gi??p agent bi???t khi n??o ph???i t??? ch???i ????? xu???t "nghe hay" nh??ng l???ch h?????ng.
- Template scaffold ????? `(ch??a ch???t ??? user ??i???n)`; ??S???a ?????i ghi r?? **ch??? user quy???t c??? M???c ????ch l???n ??i???u kho???n**, v?? "M???c ????ch c??n (ch??a ch???t) = harness ch??a xong".

**M???c ????ch c???a zemory (??i???n th???t):** l???p qu???n tr??? b??? nh??? + context cho coding agent, 2 vai ??? ??? m???t b??? n??o chung (m???i agent + web chat ??? 1 SQLite local, dedup/redact, search keyword l???n ng??? ngh??a, xuy??n project + xuy??n m??y) ??? m???t harness chu???n cho t???ng project. Tr?? tu??? l?? agent ??ang l??i terminal, zemory ch??? lo **nh??? + k??? lu???t**. Phi-m???c-ti??u: kh??ng proxy/t??? g???i model API ?? kh??ng n??n ng??? c???nh (b??? scope 2026-06-25) ?? kh??ng c??? ?????nh N???I DUNG docs (ch??? c???u tr??c + rule + harness) ?? kh??ng kho th??? hai ?? kh??ng ?????ng ngo??i ph???m vi ???????c giao.

**K??m 2 vi???c d???n:**
1. **15 file docs c??n header c?? "do not hand-edit"** ??? m??u thu???n TR???C TI???P v???i lu???t FILE WINS v???a ship (ch??nh th??? l??m session kh??c r???i). `docs render` c???p nh???t h???t; git diff x??c nh???n **ch??? header ?????i**, 0 m???t n???i dung.
2. **V?? false-positive salvage trong `renderDoc`**: n?? so `sha1(file)` v???i `doc.rendered_hash` ??? m?? `docs sync` KH??NG render n??n hash lu??n c?? ??? **m???i l???n render sau sync ?????u ????? 1 `.bak` r??c** d?? DB ???? c?? ????ng n???i dung ????. Nay so **th??n file vs th??n DB** (????ng c??u h???i c???n h???i: file c?? g?? CH??A v??o DB kh??ng?). +1 test kh??a: n???i dung ???? sync ??? render KH??NG salvage v?? v???n c??n nguy??n trong file.
...
**Bug CH?? T???, ph??t hi???n nh??? agent SasinFlow b??o `changelog import` n??i "merged 0 new" trong khi `.md` c?? 9 entry DB kh??ng c??.**

**G???c:** file do editor/PowerShell Windows ghi ra l?? **CRLF**. Parser c???t theo `"\n"` ??? m???i d??ng c??n `\r` ??? ??u??i. Trong JS, **`.` KH??NG kh???p `\r`** (n?? l?? line terminator) v?? **`$` c??ng kh??ng ?????ng tr?????c `\r`** ??? 2 regex ch??? l???c ch???t c??m:
- `parseChangelog`: `H2 = /^## (.*?)[ \t]*$/` ??? **0 entry** ??? `import` b??o "merged 0" tr??n file ?????y ???p, **kh??ng m???t l???i c???nh b??o**.
- `parseMarkdown`: `HEADING = /^(#{1,6})[ \t]+(.*?)[ \t]*$/` ??? **0 heading** ??? c??? file th??nh **1 blob `heading=NULL`**, m???t s???ch ????? chi ti???t section.

**V?? sao gi??? m???i l??? + v?? sao nguy:** lu???t c?? ("DB l?? ngu???n, c???m g?? tay") che n?? ??? file lu??n do zemory render ra (LF). V???a ?????i sang **FILE WINS** (s???a tay l?? ???????ng CH??NH) th?? m???i agent vi???t docs tr??n Windows d??nh ngay. N???ng h??n: guard salvage t??i v???a th??m c??ng **b??? v?? hi???u** (n?? d??ng `parseChangelog` ????? t??m entry ch??a merge ??? parse ra 0 th?? t?????ng kh??ng c?? g?? ????? c???u ??? **render ???? = m???t th???t**).

**S???a:** normalize CRLF???LF ngay bi??n v??o c???a c??? 2 parser (`normEol` trong `markdown.ts` + `parseChangelog`), th??n section l??u LF. Kh??ng ?????ng logic kh??c.

**Ki???m ch???ng th???t:** `D:\Zyro\Tool\SasinFlow\docs\agent\05_CHANGES.md` ??? 527 k?? t??? `\r`, 26 heading `## `, parse c?? ra **0**. Sau fix: parse ????ng.

**+2 test kh??a:** doc CRLF t??ch ????ng section (`Spec`/`Part A`/`Part B` thay v?? 1 blob) ?? changelog CRLF merge ????ng 2 entry, re-import ra 0 (ch???ng minh title kh??ng d??nh `\r` l??m h???ng dedup).

**Ghi ch?? li??n quan:** bug "8 doc l??u 1 blob" c???a ch??nh zemory **KH??NG** do CRLF (file zemory l?? LF) ??? n?? do lu???t c?? `kept DB source` kh??ng bao gi??? re-split; **FILE WINS ???? t??? ch???a** (sync gi??? b??o `02_RULES.md ??? 9 sections (file wins)`).

Gate: `npm run check` **88/88**.

## [2026-07-16] — FILE WINS: .md la nguon docs, DB chi la index dan xuat (doi luat can ban)

> ???? **Supersede:** thay quy???t ?????nh "DB l?? ngu???n s??? th???t c???a curated docs, .md ch??? l?? mirror render" (ch???t 2026-06-18, plan 02 ??0 + hi???n ph??p ??i???u 3) ??? **user quy???t 2026-07-16**: zemory ch??a ????? ???n ?????nh ????? c??? ?????nh N???I DUNG docs; n?? ch??? c??? ?????nh ???????c **c???u tr??c folder + rule chung + b??? harness**. Agent vi???t docs b??m chu???n l?? ?????.

**FILE WINS: `.md` l?? NGU???N c???a docs; DB ch??? l?? INDEX d???n xu???t** (search/sync), d???ng l???i ???????c t??? file b???t c??? l??c n??o.

**V?? sao ?????i:** lu???t c?? ("c???m g?? tay .md, ph???i qua `plan set`/`changelog add`") g??y r???i th???t ??? session kh??c ?????c AGENTS.md th???y tuy??n b??? ???? r???i quan s??t h??nh vi th???t (`docs sync` b??o `kept DB source`, s???a tay b??? nu???t) ??? kh??ng bi???t ???????ng n??o m?? l???n. R??ng bu???c ???? c??ng ch???n agent l??m vi???c t??? nhi??n trong khi gi?? tr??? th???t c???a zemory n???m ??? **khung** ch??? kh??ng ph???i ??? ch??? gi??? n???i dung.

**Code (`backend/src/docs/`):**
- `plan.ts importAll`: mirror b??? s???a tay (n???i dung file ??? b???n render t??? DB) ??? **RE-IMPORT theo file**. N???i dung kh???p ??? gi??? nguy??n DB rows (**ID section ???n ?????nh, kh??ng churn**). T??? l??nh 8 doc "1 blob" (bug ?????ng b??? c??) ngay l???n ?????u file ???????c s???a.
- `plan.ts` changelog: `docs sync` **LU??N merge t??? file** (additive theo `date+title`) ??? mirror nguy??n v???n merge 0 entry; entry vi???t tay t??? v??o DB. B??? nh??nh `hasChangelog` ch???n import.
- `changelog.ts renderChangelog`: v?? l??? h???ng th???t ??? b???n c?? ch??? salvage khi file KH??NG c?? header GENERATED, n??n **hand-edit gi??? header b??? ???? c??m = m???t d??? li???u**. Nay salvage khi file ch???a entry **ch??a c?? trong DB** (so `date+title`), kh??ng .bak-spam ??? render th?????ng.
- Header GENERATED ?????i l???i: *"hand-edits WELCOME (file wins) ??? run `zemory docs sync`"*.
- `cli docs sync` in r??: `unchanged (matches DB index)` / `N sections (file wins)` / `merged N new entr(ies)`.

**Lu???t ch???:** hi???n ph??p ??i???u 3 ghi amendment (n??u r?? l?? do + ng??y); `02_RULES` (template + zemory) th??m m???c **"Docs = FILE l?? ngu???n (FILE WINS)"**; `AGENTS.md` ??2 vi???t l???i doctrine, b??? m???c "2 LO???I docs" v???a th??m h??m tr?????c (gi??? ch??? c??n 1 lo???i: file l?? ngu???n).

**Gate:** `npm run check` **86/86** (+3 test kh??a h??nh vi m???i: sync re-import hand-edit & gi??? ID khi kh???p & t??ch section m???i ?? changelog merge t??? file ?? render salvage entry ch??a merge nh??ng kh??ng spam .bak).
