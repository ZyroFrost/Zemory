...
## [2026-07-16] — chore(harness): `docs sync` thôi là chỉ thị cho agent (chỉ tiện ích tay) + sửa header "GENERATED from DB"

**Bỏ mọi chỉ thị "chạy `docs sync`" khỏi file agent đọc** (`AGENTS §3` + `RULES §Tài liệu/§Đồng bộ/§Chốt phiên`, cả template): sửa `.md` **xong là xong** (file là nguồn), KHÔNG cần sync. `docs sync` chỉ còn tiện ích tay nếu muốn `plan search`/`changelog ls` tươi. Lý do (user chốt): agent đọc `.md` trực tiếp nên docs-index chỉ là **mirror thừa**; các chỉ thị sync khiến agent "dính" đòi chạy hoài + tốn token đọc-rồi-hỏi. **Não episodic + embed/vector + sync Drive KHÔNG đụng** (bảng khác, lệnh khác).

**Sửa header stale** dòng-1 `01_CONSTITUTION`/`02_RULES`: *"GENERATED by zemory from global_memory.db … after editing run docs sync"* (di sản trước FILE WINS — khiến tưởng `.md` sinh RA TỪ DB) → *"NGUỒN = chính file .md; DB chỉ là index dẫn xuất, không cần chạy gì"*. (Các doc khác còn header cũ — cosmetic, gỡ trong đợt bỏ docs-index.)
