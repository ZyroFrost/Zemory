...
- [ ] (nhỏ, chưa sửa) `zemory plan show <id>` in lặp dòng header 2–3 lần trước nội dung.
- [ ] **(chờ user) 3 commit chưa push** (`711cd0e` CRLF+luật Git · `005696d` hiến pháp Mục đích · `1b45fae` cửa chặn agent ngoài) — theo `02_RULES §Git`, user bảo mới đẩy.
