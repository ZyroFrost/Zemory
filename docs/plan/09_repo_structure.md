...
- **Chuẩn v2 (2026-07-10) — mở rộng phủ đủ mọi project + 2 trục sắp xếp:**
  ...
  - **UI embed (single-binary):** app CLI ship 1-file được phép embed trang UI như resource TS ở backend — GHI RÕ trong convention, KHÔNG ép tách sang `frontend/` nếu tách làm vỡ build 1-file. *(zemory TRƯỚC ĐÂY embed `ui-page.ts`; nay đã tách sang `frontend/` no-build multi-file — theo convention "UI no-build (static)" `03_STRUCTURE §5`, KHÔNG còn là ví dụ single-bin.)*
