...
- **F1. Asymmetric prompts + Matryoshka 256d + chunk message dài + MCP grade/rewrite — XONG 2026-07-12/14:** so sánh với một repo RAG khác (production-agentic-rag-course) phát hiện EmbeddingGemma là prompt-trained (query cần prefix `task: search result | query:`, document cần `title: none | text:`) mà zemory chưa dùng — thêm `embedQuery`/`embedDocBatch` với profile lưu `vec_config.profile` (stored-config-authoritative, giống pattern dims). Message >6000 ký tự giờ chunk thành cửa sổ chồng lấn (6000/500, tối đa 8, rowid tổng hợp qua `vec_map`) thay vì cắt cụt mất phần đuôi. `memory_search` (MCP) có hướng dẫn agent tự chấm + viết lại query (≤2 lần) trước khi kết luận không có — agentic loop, 0 token phía zemory. Kèm plan 12 (`attic/dead-plans/12_vector_rebuild_256.md` — HOÀN TẤT, dời khỏi `docs/plan/` 2026-07-29; vẫn tra được, kind `plan-archive`): rebuild toàn bộ DB thật ở 256d (đổi quyết định §5 cũ "768d đầy đủ") + FTS external-content + VACUUM → **DB 1141.4MB→595.1MB (−48%)**, gate `npm run check` 82/82 + `memory bench --rerank` hybrid/rerank 100% (8/8).
