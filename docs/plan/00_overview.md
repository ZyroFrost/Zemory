...
- Capture mặc định = **scheduler scan của daemon**, KHÔNG phải Stop hook; `hook install` là lối TUỲ CHỌN khi cần độ trễ thấp hơn 30 phút. Host khác cần adapter/lifecycle riêng. *(🔄 Sửa 2026-09-03: dòng cũ ở đây khai "Claude và Codex cùng dùng Stop capture" — §6 cùng file đã bác điều đó từ 2026-07-29 bằng số đo (0 máy nào cài hook mà vẫn nạp 6.882 tin/2 ngày), nhưng dòng này ở mục "đã chốt" thì không ai sửa. Một file nói HAI câu ngược nhau, mà câu sai lại nằm ở mục nghe có thẩm quyền hơn.)*
...
## 10. Bước kế tiếp
Trình tự đã hoàn tất qua 2026-06-30: **RAG semantic** (plan 05 — embed EmbeddingGemma + sqlite-vec + hybrid trên RRF), **MCP recall** (`memory_search`/`memory_show`/`plan_search`/`plan_show`) và **retention/privacy core** (`memory backup/restore/forget/redact`, export/import encrypted đã có). Compression đã BỎ khỏi scope (2026-06-25). Bước kế tiếp hợp lý: source-transcript tombstone nếu cần quên tuyệt đối, rồi code map / adapter host mới. *(🔄 Sửa 2026-09-03: bỏ vế "xác minh Stop hook runtime thật" — §6 đã xác minh xong 2026-07-29 và kết luận capture KHÔNG đi qua Stop hook; để nó nằm đây là mời phiên sau đi làm lại một việc đã đóng.)* Mọi lớp mới phải có migration, regression test, health check và fallback trước khi bật mặc định.
