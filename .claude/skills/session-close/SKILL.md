...
3. **SOÁT `05_TODO` = ĐO LẠI, không đọc rồi chép** (`02_RULES §Hành xử`, luật áp **MỌI LÚC** — chốt phiên chỉ là một trường hợp): mỗi mục là một KHẲNG ĐỊNH VỀ TRẠNG THÁI. Kiểm được bằng code ⇒ grep/chạy/đếm; là quyết định ⇒ `memory search --all` (quyết định hay nằm ở phiên khác, thậm chí **repo khác**); tra không ra ⇒ ghi "chưa xác minh được", KHÔNG mặc định "chưa làm". Mục quá 7 ngày = NGHI NGỜ. **Đo 2026-08-05: soát 58 mục ra 11 sai (~19%)** — mục đã build vẫn mang `[ ]`, mục agent tự bịa vì thấy triệu chứng rồi phán nguyên nhân.
