...
1. **Chưa có `docs/.harness.json`?** →
   ...
   - Rồi chạy: **APP** → `zemory init` · **NON-APP** → `zemory init --non-app` → `zemory doctor` xanh. (Thiếu lệnh `zemory` → cài TỪ MÃ NGUỒN: `git clone https://github.com/ZyroFrost/Zemory` → **`node backend/scripts/fetch-prebuilds.mjs`** → `npm install` → `npm run build` → `npm link`. **Bước đầu chạy TRƯỚC `npm install`, đừng bỏ**: `better-sqlite3` tải nhị phân dựng sẵn từ `github.com`, mạng chặn/bóp host đó (đo 2026-08-13: lọt **1/10** lượt, `api.github.com` **10/10**) thì nó rơi về `node-gyp` và **clone sạch không dựng được**; script kéo đúng file đó qua đường API rồi đặt vào cache. Không có lifecycle hook nào chạy đủ sớm — npm chạy install script của dependency TRƯỚC `preinstall` của gói gốc. **`npm i -g zemory` KHÔNG chạy** — gói chưa publish lên npm. **`npm i -g github:…` cũng không** — cài global không kéo devDependencies nên thiếu `tsc`.) Đã có `.harness.json` → bỏ qua bước này (profile đã chốt trong đó).
