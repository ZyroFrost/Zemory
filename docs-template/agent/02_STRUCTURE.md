...
| **auto-dịch** (sửa 1 ngôn ngữ → tự sinh còn lại) | engine → `ai/` (LLM) \| `integrations/` (DeepL/Google) · orchestrate → `services/` · file en/vi → `locales/` · trigger → `scripts/` (build) \| `jobs/` (runtime) \| `.husky/` (commit) |
