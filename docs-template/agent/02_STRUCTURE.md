...
| **check bản mới / tự cập nhật (version-up)** | `backend/src/update/` (check Releases/registry + tải + apply). Package-manager phân phối (npm/pip) → có thể KHÔNG cần, ghi rõ lý do |
