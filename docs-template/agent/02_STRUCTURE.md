...
│   ├── scripts/                    [opt]   script dev/build/ops/DEPLOY của mình (đẩy bản mới lên máy đích/VM)
...
| **check bản mới / tự cập nhật (version-up TỰ ĐỘNG)** | `backend/src/update/` (check Releases/registry + tải + apply). Package-manager phân phối (npm/pip) → có thể KHÔNG cần, ghi rõ lý do |
| **chốt version + deploy lên máy đích/VM (THỦ CÔNG)** | git tag (vX.Y.Z) → `dist/`(build) → `backend/scripts/deploy.*` → backup bản cũ TRÊN máy đích về `attic/` TRƯỚC KHI ghi đè |
...
2 KIỂU version-up     ① TỰ ĐỘNG (app tự check+tải+apply) → backend/src/update/ (rule trên). ② THỦ CÔNG (người quyết định "chốt bản X, up lên máy đích/VM") → KHÔNG cần code update/ gì cả, dùng ĐÚNG hạ tầng có sẵn: **chốt version** = git tag (vX.Y.Z) khớp bump manifest → **build** → dist/ → **deploy** = backend/scripts/deploy.* đẩy lên máy đích → TRƯỚC KHI ghi đè, kéo bản ĐANG CHẠY trên máy đích về backup (attic/ hoặc thư mục backup có ngày-tháng trên chính máy đích) → rollback nếu bản mới hỏng. Đây chính là mục đích gốc của attic/+dist/+git-tag, KHÔNG phải concern mới
Backup deploy = 2 CHIỀU  KHÔNG chỉ push 1 chiều (local→VM). Máy đích ĐÃ CÓ SẴN backup từ lần deploy trước → **verify TRƯỚC:** so khớp (checksum/diff) backup-trên-VM với attic/ local — LỆCH nghĩa là có gì đó chỉnh tay ngoài luồng deploy, phải điều tra TRƯỚC KHI đè bản mới. Deploy xong → kéo bản-vừa-thay (giờ là backup mới nhất) VỀ attic/ local, giữ 2 bên luôn khớp. Cùng nguyên lý bidirectional-verify như `brain sync`/`share.ts` (additive merge, không tin mù, luôn đối chiếu trước khi hợp nhất) — áp lại cho backup deploy thay vì brain data
...
## 5. Phạm vi áp dụng
- **ÁP:** hầu hết app estate (UI + server-side) — desktop WebView2 (DuAnA), web app, tool có cockpit (zemory), AI/data project, monorepo. Áp gần như không đổi cấu trúc.
