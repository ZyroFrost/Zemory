...
│   │   ├── store/                  [opt]   DATA-ACCESS: driver connection + schema (DB remote/nội-bộ/cloud ĐỀU đây)
...
│   │   ├── auth/                   [opt]   authentication + authorization + jwt + oauth + permission (KHÁC middleware)
│   │   ├── vault/                  [opt]   quản lý credential (mã hóa) — CODE ở đây; FILE mã hóa → data/secrets/
...
| auth / login / permission | `backend/src/auth/` (KHÁC `middleware/`) |
