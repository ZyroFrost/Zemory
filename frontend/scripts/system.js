...
// Cắt CƠ HỌC giữ hành vi; dời hàm giữa file là việc của đợt sau. Xem 06_CHANGES.
  ...
  function buildRepoBlock(st){
    ...
    function deadPrompt(list,unproven){
      unproven=unproven||[];
      var parts=[];
      if(list.length){
      ...
      // KHÔNG KẾT LUẬN ĐƯỢC (user 2026-09-17): zemory chỉ thấy chuỗi không giải được; nó là con trỏ hỏng, placeholder
      // hay thứ đã xoá thì phải HỎI CHÍNH REPO ĐÓ — prompt nói rõ ba nhánh xếp loại, không cho đoán đích.
      if(unproven.length){
        var uh=t('fix.promptUnproven').replace('{n}',unproven.length);
        var ul=unproven.map(function(x){var smp=(x.sample||[]).slice(0,3).join(' · ');return '- '+(x.root||'')+'  ('+(x.unproven||0)+')'+(smp?('  ⟵ '+smp):'');}).join('\n');
        parts.push(uh+'\n'+ul+'\n\n'+t('fix.promptUnprovenSteps'));
      }
      return parts.join('\n\n');
    }
    var dd=Z.updDead||[], uu=Z.updUnproven||[];
