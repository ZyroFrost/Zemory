...
// Cắt CƠ HỌC giữ hành vi; dời hàm giữa file là việc của đợt sau. Xem 06_CHANGES.
  ...
  function recallRow(h,i){var sc=rScore(h),na=h.atts?h.atts.length:0;return '<div class="hit'+(h.id===rSel?' sel':'')+'" data-hit="'+h.id+'" data-sess="'+stdEsc(h.sessionId||'')+'"><div class="top">'+(sc?'<span class="score">'+sc+'</span> ':'')+stdEsc(h.role||'msg')+' · '+stdEsc(zProjName(h.project))+' <span class="tagx">'+stdEsc(h.source||'session')+'</span>'+(na?'<span class="att-n">🖼️'+na+'</span>':'')+'<span class="openfull" data-openfull="'+stdEsc(h.sessionId||'')+'" title="'+t('recall.openFull')+'">⤢</span></div><div class="txt">'+stdEsc(String(h.snippet||'').slice(0,170))+'</div><div class="dt">'+String(h.timestamp||'').slice(0,10)+' · #'+h.id+'</div></div>';}
