...
// Cắt CƠ HỌC giữ hành vi; dời hàm giữa file là việc của đợt sau. Xem 06_CHANGES.
  ...
  function renderSessList(){
    ...
    box.innerHTML=rows.length?rows.map(function(s){var ti=(s.title&&String(s.title).trim())||t('sess.untitled');return '<div class="sys-li'+(svCur===s.sessionId?' on':'')+'" data-sess="'+stdEsc(s.sessionId)+'" style="align-items:flex-start"><span class="sxn" style="white-space:normal">'+stdEsc(String(ti).slice(0,64))+(s.atts?' <span class="att-n">🖼️'+s.atts+'</span>':'')+'<div class="muted" style="font-size:10.5px;margin-top:1px">'+stdEsc(zProjName(s.project))+' · '+stdEsc(s.source||'')+' · '+zN(s.messages)+' msg</div></span><span style="font-size:10px;color:var(--text-faint);flex:0 0 auto;margin-left:6px;text-align:right">'+relTime(s.endedAt).big+'<span class="ctx-b" data-ctxfor="'+stdEsc(s.sessionId)+'" style="display:block;margin-top:2px"></span></span></div>';}).join(''):'<div class="muted" style="font-size:12px">'+t('sess.none')+'</div>';
    fillCtxBadges(rows);
  }
  // ── Badge CONTEXT ─────────────────────────────────
