...
5. Global Memory tiếp tục ingest transcript và cung cấp recall qua MCP/CLI, không cần proxy.
...
Session cuối của `headroom-custom` đã chốt hai điểm tại message `#46488` và `#46495` trong Global Memory:
...
MCP memory tools  --> search/show đúng phần cần recall
```
...
- Slot `memory` vẫn do Global Memory sở hữu.
- Slot `search` vẫn dùng FTS của memory; semantic local có thể thay provider sau nhưng không chạy song song như một nguồn sự thật khác.
...
- `zemory_memory_search(query, project?, limit?)`
- `zemory_memory_show(id)`
...
- Artifact metadata liên kết với `session_id`, `project_root`, tool và timestamp để memory search có thể dẫn tới output gốc.
...
| [claudioemmanuel/squeez](https://github.com/claudioemmanuel/squeez) | Apache-2.0, hook adapters đa-host, `updatedToolOutput`, cross-call dedup, MCP và eval corpus | Nguồn tốt nhất để extend/tham khảo adapter và eval; không bật persona/session-memory riêng để tránh cạnh tranh Global Memory |
...
### Giai đoạn E — MCP hợp nhất compression + memory

Thêm stdio MCP server với năm tool ổn định. `memory_search/show` đọc đúng global DB hiện tại; `output_search/show` đọc artifact. Không có memory DB thứ hai.
...
**zemory quota-safe admission gateway + artifact progressive disclosure + Global Memory, không model proxy.** Compression là capability chủ chốt và là ưu tiên triển khai số 1 của session kế tiếp.
...
- dùng chung Global Memory và host adapter nhưng không rewrite conversation history;
