...
> **Trạng thái: HOÀN TẤT (2026-07-14) — xem docs/plan/12_vector_rebuild_256.md.** User đã duyệt (2026-07-12, không backup — DB là lớp dẫn xuất). Bước 2 (cắt 768d→256d tại chỗ) được thay bằng rebuild thẳng ở 256d trong plan 12, vì đằng nào cũng phải re-embed toàn bộ để đổi sang asymmetric Gemma prompts. Kết quả thật: DB 1141.4MB → 595.1MB (giải phóng 546.3MB, ~48%), 94384 vector, gate npm run check 82/82 + memory bench hybrid/rerank 100% (8/8), 0 mất dữ liệu.
...
- TRƯỚC khi mổ: `memory backup` (bản .db nguyên) — bắt buộc.
- SAU mỗi bước: `npm run check` + `zemory memory bench` (gate: hybrid recall@3 ≥ FTS; mốc hiện tại 100% 8/8) + spot-check search thật.
...
- Còn đo dở: tốc độ embed hằng ngày thực tế (mẫu 5.813 msg mới) — session trước đang chạy nền, có thể bị ngắt khi đổi session → **session sau chạy `zemory memory embed --all` (vô hại, incremental) + bấm giờ để ra số phút/ngày thật.**
